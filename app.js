...
app.get("/registererichong", function(req, res) {
	...
	user.save((err, u) => {
		...
		res.send('User added: erichong')
	})
})

app.get("/registerjohnblaketh", function(req, res) {
	const user = new User({
		name: "johnblaketh",
		access_token: 'AQURoks4UzLJdhdKogVLXNrOOlKllnG98IY7jkQaIf9KVA35m6_K7rLUf1zRNxmAkYVZpDs5zDH4oy-bX_otJTVthddvcco80A5VGJw4S4I4ci75Z2LAUR0pwaQVarKhFZLZYc1JNDJZOX5FwjuZmXEAstNDLwCTrxxzPePxKPMzDafJDOo'
	});
	user.save((err, u) => {
		if (err) return console.error(err);
		console.log('User added: johnblaketh');
		res.send('User added: johnblaketh')
	})
})
...
app.get("/linkedin", function(req, res) {
	request.post({
	  	headers: {'content-type' : 'application/x-www-form-urlencoded'},
	  	url:     'https://www.linkedin.com/oauth/v2/accessToken',
	 	form:    {'code': 'AQSAioWfYLFT9XAcI9pNLnKhLZSkNOqK_uiED3rfc_NrAE9F3d1TrYIswW0SxOLr3LGdQmTK6EHJk_fdPnHt3b9HC6SlR_C2UuipxzO5wU8Cwgrgpl8',
	 'grant_type': 'authorization_code',
